...
interface FancyMarkdownProps {
  content: string;
}

export const FancyMarkdown: React.FC<FancyMarkdownProps> = ({ content }) => {
  ...
  const renderLine = (line: string, index: number) => {
    ...
    if (orderedMatch) {
      return (
        <p key={index} className="font-bold text-white mt-2">
          {orderedMatch[0]}
        </p>
      );
    }
    // Subpoint (indented dash)
    const subpointMatch = line.match(/^\s{2,}[-*•]\s+(.*)/);
    if (subpointMatch) {
      return (
        <p key={index} className="ml-6 text-sm text-white/80">
          {parseInline(subpointMatch[1])}
        </p>
      );
    }
    // Headers
